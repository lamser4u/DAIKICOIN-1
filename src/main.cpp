...
#if defined(NDEBUG)
# error "DAIKICOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x903f16432478991de3f81caaa37d154201930b7a3013be674e1699fd2ff7b234");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DAIKICOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DAIKICOIN Signed Message:\n";
...
{
    // DAIKICOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 2143 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 2100140000 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DAIKICOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DAIKICOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1491927610, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "NEW COIN FROM JAPAN";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2143 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d7810c774d0f09f728756360aa64ae83131062009ed633e3fea6c9353b79147bd3f79f479077d512cf146a91eea30bf44402ba533bbcecb0071078a4096ff8c1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1491927610;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3225914;
        ...
        assert(block.hashMerkleRoot == uint256("0x47d5cfee6f4f0a30b0718679dd3358770382caf23b33dd7266118f221246f49f"));
...
unsigned char pchMessageStart[4] = { 0x3a, 0x6f, 0x07, 0x77 }; // DAIKICOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DAIKICOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DAIKICOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DAIKICOINMiner(CWallet *pwallet)
{
    printf("DAIKICOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("daikicoin-miner");
    ...
    try { loop {
        ...
        printf("Running DAIKICOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DAIKICOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DAIKICOINMiner, pwallet));
}
